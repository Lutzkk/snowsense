test_that("snow_cover_fraction works for both methods", {
  ...
  scf_sal <- snow_cover_fraction(x, bands=list(green=3, swir=12), method="salomonson")
  scf_sig <- snow_cover_fraction(x, bands=list(green=3, swir=12), method="sigmoid")
  ...
  expect_error(snow_cover_fraction(x, bands=list(green=3, swir=12), method="some random method that does not exist"))
})
